...
function ratmag()
	...
	local field_captions = {"id", "is_vanilla", "is_tog_patched", "magazine", "caliber"}
	local csv = {}
	...
	ForEachPreset("InventoryItemCompositeDef", function(w)
		...
		if IsKindOf(item, "Firearm") then
			local line = {id = item.class, is_vanilla = item.is_vanilla_firearm, is_tog_patched = item.is_tog_patched,
			magazine = item.Magazine or "_null",
			caliber = item.Caliber}
			--print(line)
